...
jum_table=0;
    ...
    if(ids!=''){
        $.ajax({
            type: 'ajax',
            url: base_url(`freelance/tampilkanData/`)+ids,
            async: false,
            dataType: 'json',
...
jum_table=0;
    ...
    if(ids!=''){
        if(tipe=='1'||tipe=='3'||tipe=='5'){
            $.ajax({
                type: 'ajax',
                url: base_url(`finance/tampilkanData/`)+ids,
                async: false,
                dataType: 'json',
                success: function (data) {
                    for(i=0; i<data.po.length; i++){
            ...
            $.ajax({
                type: 'ajax',
                url: base_url(`finance/tampilkanDataitem/`)+ids,
                async: false,
                dataType: 'json',
                success: function (data) {
                    for(i=0; i<data.po.length; i++){
...
cost = [];
    ...
    if(ids=='1'||ids=='3'||ids=='5'){
        $.ajax({
            type: 'ajax',
            url: base_url(`finance/tampilkanDataPoWord/`),
            async: false,
            dataType: 'json',
            success: function (data) {
                addoption();
        ...
        type: 'ajax',
        url: base_url(`finance/tampilkanDataPoItem/`),
        async: false,
        dataType: 'json',
        success: function (data) {
            addoption();
